...
object CalendarOverridesStore {
    ...
    data class Vacation(val from: LocalDate, val to: LocalDate, val description: String)
    ...
    suspend fun addHoliday(context: Context, date: LocalDate, description: String) {
    ...
    suspend fun addVacation(context: Context, from: LocalDate, to: LocalDate, description: String) {
    ...
    fun toRawHoliday(h: Holiday): String = "${h.date.format(fmt)}|${h.description}"
    fun toRawVacation(v: Vacation): String = "${v.from.format(fmt)}..${v.to.format(fmt)}|${v.description}"
