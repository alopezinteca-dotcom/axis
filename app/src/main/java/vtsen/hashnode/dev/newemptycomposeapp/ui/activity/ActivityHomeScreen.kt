...
fun ActivityHomeScreen(
    ...
    // Compatibilidad con VM antiguo (si tu VM ya lo hace automático, no pasa nada)
    ...
    // =========================
    // 5) EXPORT CSV UNIFICADO  ✅ FIX2: exportUnifiedCsvIO LOCAL
    // =========================
    ...
    val folderPickerLauncher = rememberLauncherForActivityResult(
        ...
        if (uri != null) {
            // Persistimos permisos para no romper al día siguiente
            val takeFlags = Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_WRITE_URI_PERMISSION
            try {
                context.contentResolver.takePersistableUriPermission(uri, takeFlags)
            } catch (_: SecurityException) {
                // Ignorar si el proveedor no lo permite
            }
...
// =========================
    // UI: gestión calendario saveRange(d, newTo)    // UI: gestión calendario (📅)
                        }
