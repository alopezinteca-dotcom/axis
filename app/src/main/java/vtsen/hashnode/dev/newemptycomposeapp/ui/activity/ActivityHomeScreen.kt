...
// ─────────────────────────────────────────────────────────────────────────────
// CORRECCIONES APLICADAS EN ESTE FICHERO:
//
//  1. "importimport" (línea 2 original) → "import"  [error de compilación]
//  2. Eliminados imports duplicados de Dispatchers/withContext/JSONObject que
//     ya no se usan aquí (la lógica se movió al ViewModel en V2).
//  3. El backup parse + prune ya vive en ActivityViewModel / BackupManager;
//     las referencias internas se delegan al ViewModel.
//  4. El bloque de GPS duplicado de NewTravelScreen no afecta a este fichero,
//     pero se añade comentario para que no se reimplemente aquí accidentalmente.
// ─────────────────────────────────────────────────────────────────────────────
...
fun ActivityHomeScreen(
    ...
    // ── CORRECCIÓN: 350.0 y 8.0 deberían venir de Settings; aquí son constantes
    //    temporales hasta que el ViewModel exponga esos valores configurables.
    val objetivoDiarioEuros = 350.0
    val horasPorDia = 8.0

    val totalFacturarObjetivo by remember(diasLaborables) {
        derivedStateOf { objetivoDiarioEuros * diasLaborables }
    }
    val horasObjetivo by remember(diasLaborables) {
        derivedStateOf { horasPorDia * diasLaborables }
    }
    val deltaHoras by remember(horasObjetivo, horasImputadasPeriodo) {
        derivedStateOf { horasObjetivo - horasImputadasPeriodo }
    }
    ...
    // ── 5) Drive / Export / Backup — delegado al ViewModel ────────────────
    ...
    val restorePickerLauncher = rememberLauncherForActivityResult(
        ...
        coroutineScope.launch {
            try {
                // La lectura y parsing se delega al ViewModel
                val snapshot = viewModel.readAndParseBackup(context, uri)
                pendingSnapshot = snapshot
                showRestoreConfirm = true
            } catch (e: Exception) {
                snackbarHostState.showSnackbar(
                    "❌ No se pudo leer el backup: ${e.localizedMessage ?: "desconocido"}"
                )
            }
        }
    }

    // ── Export all: delega al ViewModel ───────────────────────────────────
    fun doExportAll() {
        ...
        coroutineScope.launch {
            isExporting = true
            val result = viewModel.exportAll(
                context = context,
                axisTreeUri = axis,
                periodTravels = periodTravels,
                stopsInPeriod = stopsInPeriod,
                dailyCsvName = dailyCsvName(),
                monthlyCsvName = monthlyCsvName(periodKey()),
                todayKey = todayKey(),
                periodKey = periodKey()
            )
            isExporting = false
            ...
            confirmButton = {
                TextButton(onClick = {
                    ...
                    coroutineScope.launch {
                        isExporting = true
                        try {
                            viewModel.restoreFromSnapshot(snap)
                            snackbarHostState.showSnackbar("✅ Restauración completa")
                        } catch (e: Exception) {
                            snackbarHostState.showSnackbar(
                                "❌ Error restaurando: ${e.localizedMessage ?: "desconocido"}"
                            )
                        } finally {
                            isExporting = false
                        }
    ...
    if (showEditDialog && editingTravelId != null) {
        ...
        AlertDialog(
            ...
            dismissButton = {
                TextButton(onClick = { editWarning = null; showEditDialog = false }) { Text("Cancelar") }
            }
        )
    }
                ...
                if (axis == null) {
                    Card(
                        ...
                        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            Text(
                                "⚠️ Carpeta AXIS no configurada",
                                fontWeight = FontWeight.Bold,
                                color = MaterialTheme.colorScheme.error
                            )
                            Text(
                                "Pulsa abajo y elige en Drive tu carpeta AXIS.",
            ...
            Column(
                ...
                LazyColumn(
                    ...
                    items(daysInRange) { day ->
                        ...
                        val currentDay = current?.startTimestamp?.let {
                            BillingPeriodStore.millisToLocalDateOrToday(it, zone)
                        }
        ...
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            Row(
                ...
                Column(Modifier.weight(1f)) {
                    Text(
                        "$date · ${travel.origin} → ${travel.destination}$warning",
...
private fun countWeekdaysInSet(from: LocalDate, to: LocalDate, dates: Set<LocalDate>): Int {
    ...
    return dates.count {
        it in from..to && it.dayOfWeek != DayOfWeek.SATURDAY && it.dayOfWeek != DayOfWeek.SUNDAY
    }
}

private fun expandVacationDates(
